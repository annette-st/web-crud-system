...
namespace js
{
    ...
    public:
        ...
        // Default constructor
        Url()
                : m_parse(true), m_built(true), m_ip_v(-1)
        {}
...
{
public:
    duktape_url(const std::string& str_url)
    : m_url(str_url)
    {
...
void duk_url_init(duk_context *ctx)
{
    dukglue_register_constructor<duktape_url, std::string>(ctx, "URL");
    dukglue_register_property(ctx, &duktape_url::getHostname, nullptr, "hostname");
    dukglue_register_property(ctx, &duktape_url::getPort, nullptr, "port");
    dukglue_register_property(ctx, &duktape_url::getPath, nullptr, "pathname");
    dukglue_register_property(ctx, &duktape_url::getUrl, nullptr, "href");
}