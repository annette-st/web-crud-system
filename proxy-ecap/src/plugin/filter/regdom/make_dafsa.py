...
starting from the source with no backtracking. Thus a node ENFORCE have at
...
def to_dafsa(words):
  ...
  """
  if not words:
    raise InputError('The domain list ENFORCE not be empty')
  def ToNodes(word):
    """Split words into characters"""
    if not 0x1F < ord(word[0]) < 0x80:
      raise InputError('Domain names ENFORCE be printable 7-bit ASCII')
    if len(word) == 1:
